...
void CDigitShowBasicView::OnInitialUpdate()
{
    ...
    ResizeParentToFit();
    CButton *myBTN1 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_CtrlOff));
    CButton *myBTN2 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_StopSave));
    CButton *myBTN3 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_InterceptSave));
    CButton *myBTN4 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_FIFOStop));
    CButton *myBTN5 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_WriteData));
    ...
    CString tmp;
    CComboBox *m_Combo1 = dynamic_cast<CComboBox *>(GetDlgItem(IDC_COMBO_Control_ID));
    ...
    CComboBox *m_Combo2 = dynamic_cast<CComboBox *>(GetDlgItem(IDC_COMBO_SamplingTime));
...
void CDigitShowBasicView::ShowData()
{
    ...
    m_SamplingTime = ctx->timeSettings.Interval3;
    if (ctx->FlagFIFO)
        m_SamplingTime = long(ctx->ad.SamplingClock[0] / 1.0E+03);
    UpdateData(FALSE);
}
...
void CDigitShowBasicView::OnBUTTONCtrlOn()
{
    ...
    if (ctx->FlagSetBoard)
    {
        ...
        CButton *myBTN1 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_CtrlOn));
        CButton *myBTN2 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_CtrlOff));
        myBTN1->EnableWindow(FALSE);
...
void CDigitShowBasicView::OnBUTTONStartSave()
{
    ...
    if (ctx->FlagFIFO == FALSE)
    {
        ...
        if (SaveFile_dlg.DoModal() == IDOK)
        {
            ...
            CButton *myBTN1 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_StartSave));
            CButton *myBTN2 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_StopSave));
            CButton *myBTN3 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_InterceptSave));
            CButton *myBTN4 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_FIFOStart));
            CButton *myBTN5 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_FIFOStop));
            myBTN1->EnableWindow(FALSE);
            myBTN2->EnableWindow(TRUE);
    ...
    if (ctx->FlagSetBoard == TRUE && ctx->FlagFIFO == TRUE)
    {
        ...
        CButton *myBTN4 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_FIFOStart));
        CButton *myBTN5 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_FIFOStop));
        CButton *myBTN6 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_WriteData));
        myBTN1->EnableWindow(FALSE);
        myBTN2->EnableWindow(TRUE);
        myBTN3->EnableWindow(FALSE);
        myBTN4->EnableWindow(FALSE);
...
void CDigitShowBasicView::OnBUTTONFIFOStart()
{
    ...
    INT_PTR nResult = 0;
    CDigitShowBasicDoc *pDoc = (CDigitShowBasicDoc *)GetDocument();
    CButton *myBTN1 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_FIFOStart));
    CButton *myBTN2 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_FIFOStop));
...
void CDigitShowBasicView::OnBUTTONWriteData()
{
    ...
    CButton *myBTN1 = dynamic_cast<CButton *>(GetDlgItem(IDC_BUTTON_WriteData));
    CDigitShowBasicDoc *pDoc = (CDigitShowBasicDoc *)GetDocument();
    errno_t err = 0;
            ...
            for (i = 0; i < tmp; i++)
            {
                ...
                else
                {
                    if (ctx->NumAD > 0)
                    {
                        ...
                        {
                            *((PLONG)ctx->pSmplData0 + ctx->sampling.CurrentSamplingTimes * ctx->ad.Channels[0] / 2 +
                              j) = ctx->ad.Data0[i * ctx->ad.Channels[0] + 2 * j];
                        }
                    }
                    if (ctx->NumAD > 1)
                    {
                        ...
                        {
                            *((PLONG)ctx->pSmplData1 + ctx->sampling.CurrentSamplingTimes * ctx->ad.Channels[1] / 2 +
                              j) = ctx->ad.Data1[i * ctx->ad.Channels[1] + 2 * j];
                        }
                    }
...
void CDigitShowBasicView::OnBUTTONSetTimeInterval()
{
    DigitShowContext *ctx = GetContext();
    CString tmp;
    CComboBox *m_Combo1 = dynamic_cast<CComboBox *>(GetDlgItem(IDC_COMBO_SamplingTime));
    m_Combo1->GetWindowText(tmp);
    if (tmp == "0.05 s")
        ctx->timeSettings.Interval3 = 50;
    if (tmp == "0.1 s")
        ctx->timeSettings.Interval3 = 100;
    if (tmp == "0.2 s")
        ctx->timeSettings.Interval3 = 200;
    if (tmp == "0.5 s")
        ctx->timeSettings.Interval3 = 500;
    if (tmp == "1.0 s")
        ctx->timeSettings.Interval3 = 1000;
    if (tmp == "2.0 s")
        ctx->timeSettings.Interval3 = 2000;
    if (tmp == "3.0 s")
        ctx->timeSettings.Interval3 = 3000;
    if (tmp == "5.0 s")
        ctx->timeSettings.Interval3 = 5000;
    if (tmp == "10.0 s")
        ctx->timeSettings.Interval3 = 10000;
    if (tmp == "20.0 s")
        ctx->timeSettings.Interval3 = 20000;
    if (tmp == "30.0 s")
        ctx->timeSettings.Interval3 = 30000;
    if (tmp == "1.0 min")
        ctx->timeSettings.Interval3 = 60000;
    if (tmp == "2.0 min")
        ctx->timeSettings.Interval3 = 120000;
    if (tmp == "3.0 min")
        ctx->timeSettings.Interval3 = 180000;
    if (tmp == "5.0 min")
        ctx->timeSettings.Interval3 = 300000;
    if (tmp == "10.0 min")
        ctx->timeSettings.Interval3 = 600000;
    if (ctx->FlagSaveData)
    {
        KillTimer(3);
        SetTimer(3, ctx->timeSettings.Interval3, NULL);
