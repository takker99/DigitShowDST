...
void CDigitShowBasicDoc::OpenBoard()
{
	...
	if(ctx->FlagSetBoard){
		...
		for(i=0;i<ctx->NumAD;i++){
			...
			ctx->Ret = AioSetAiRangeAll ( ctx->ad.Id[i], 1 );	// (-5V, 5V)
		...
		for(i=0;i<ctx->NumDA;i++){
			...
			ctx->Ret = AioSetAoRangeAll ( ctx->da.Id[i] , 50 );	// 0 - 10V
...
void CDigitShowBasicDoc::Cal_Param()
{
	...
	ctx->volume=SpecimenData->Volume[0]- ctx->Phyout[4];				// Current volume in drain condition
	...
	if(SpecimenData->VLDT1[0]>0.0 && ctx->Phyout[5]>0.0) {
		ctx->phys.eLDT1=-log(ctx->Phyout[5]/SpecimenData->VLDT1[0])*100.0;	// True LDT Strain (%)
	}
	else{
		ctx->phys.eLDT1=0.0;
	}
	if(SpecimenData->VLDT2[0]>0.0 && ctx->Phyout[6]>0.0) {
		ctx->phys.eLDT2=-log(ctx->Phyout[6]/SpecimenData->VLDT2[0])*100.0;	// True LDT Strain (%)
	}
	else{
		ctx->phys.eLDT2=0.0;
	}
	...
	ctx->phys.e_sr= ctx->Phyout[3];					// Cell Effective Stress (kPa)
	...
	case 3:
		...
		// ControlData[3].sigma[0];		Limiter
			...
			if(ControlData[3].flag[0]==TRUE){		// Creep
				if(ControlData[3].MotorCruch == 0 ){
					ctx->DAVout[ctx->daChannel.MotorCruch]=0.0f;		// Cruch: Down
					if( ctx->phys.q>=ControlData[3].q+ctx->errTol.StressExt)	ctx->DAVout[ctx->daChannel.MotorSpeed]=0.0f;	// RPM->0 
				}
				if(ControlData[3].MotorCruch == 1 ){
					ctx->DAVout[ctx->daChannel.MotorCruch]=5.0f;		// Cruch: Up
					if( ctx->phys.q<=ControlData[3].q+ctx->errTol.StressCom)	ctx->DAVout[ctx->daChannel.MotorSpeed]=0.0f;	// RPM->0
				}
			}
			DA_OUTPUT();
		}
	...
	case 4:
		...
		// ControlData[4].sigma[0];		Limiter
			...
			if(ControlData[4].flag[0]==TRUE){		// Creep
				if(ControlData[4].MotorCruch == 0 ){
					ctx->DAVout[ctx->daChannel.MotorCruch]=0.0f;		// Cruch:Down
					if( ctx->phys.q>=ControlData[4].q+ctx->errTol.StressExt)	ctx->DAVout[ctx->daChannel.MotorSpeed]=0.0f;	// RPM->0 
				}
				if(ControlData[4].MotorCruch == 1 ){
					ctx->DAVout[ctx->daChannel.MotorCruch]=5.0f;		// Cruch:Up
					if( ctx->phys.q<=ControlData[4].q+ctx->errTol.StressCom)	ctx->DAVout[ctx->daChannel.MotorSpeed]=0.0f;	// RPM->0
				}
			}
			DA_OUTPUT();
		}
	...
	case 7:
		{ 
			...
			if(ControlData[7].sigma[1] == ControlData[7].e_sigma[1]){
				...
				if( ctx->phys.e_sa > ControlData[7].e_sigma[0]+ctx->errTol.StressCom)		ctx->DAVout[ctx->daChannel.MotorCruch]=5.0f;	// Cruch:Up
				else if( ctx->phys.e_sa < ControlData[7].e_sigma[0]+ctx->errTol.StressExt)	ctx->DAVout[ctx->daChannel.MotorCruch]=0.0f;	// Cruch:Down
...
{
	DigitShowContext* ctx = GetContext();
	ctx->DAVout[ctx->daChannel.MotorSpeed]=0.0f;	//Motor Speed->0
	DA_OUTPUT();
}
...
void CDigitShowBasicDoc::MLoading_Stress()
{
	...
	ctx->DAVout[ctx->daChannel.Motor]=5.0f;			// Motor: On
	ctx->DAVout[ctx->daChannel.MotorSpeed]=float(ctx->cal.DA_a[ctx->daChannel.MotorSpeed]*ctx->controlFile.Para[ctx->controlFile.CurrentNum][1]+ctx->cal.DA_b[ctx->daChannel.MotorSpeed]);	// Motor_Speed
