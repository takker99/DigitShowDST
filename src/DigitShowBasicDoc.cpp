...
void CDigitShowBasicDoc::Control_DA()
{
    ...
    case 15: {
        if (ctx->controlFile.CurrentNum >= 0 && ctx->controlFile.CurrentNum < 128)
        {
            ...
            if (ctx->controlFile.Num[ctx->controlFile.CurrentNum] == 1)
                MLoading_Stress();
            if (ctx->controlFile.Num[ctx->controlFile.CurrentNum] == 2)
                MLoading_Strain();
            if (ctx->controlFile.Num[ctx->controlFile.CurrentNum] == 3)
                CLoading_Stress();
            if (ctx->controlFile.Num[ctx->controlFile.CurrentNum] == 4)
                CLoading_Strain();
            if (ctx->controlFile.Num[ctx->controlFile.CurrentNum] == 5)
                Creep();
            if (ctx->controlFile.Num[ctx->controlFile.CurrentNum] == 6)
                LinearEffectiveStressPath();
            if (ctx->controlFile.Num[ctx->controlFile.CurrentNum] == 7)
                Creep2();
            DA_OUTPUT();
        }
    }
